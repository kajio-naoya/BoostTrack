...
def extract_oneedge(input_file, output_file, is_in_area1, is_in_area2):
    ...
    for person_id, df_person in df_per_person:
        start_point = df_person[df_person["frame"] == df_person["frame"].min()][["x", "y"]].values[0]
        end_point = df_person[df_person["frame"] == df_person["frame"].max()][["x", "y"]].values[0]
...
if __name__ == "__main__":
    input_file = "output/sample_movie_1020/sample_movie_postprocessed.csv"
    output_file = "output/sample_movie_1020/sample_movie_postprocessed_oneedge.csv"
    def is_in_area1(x, y):
